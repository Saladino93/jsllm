...
Monkey-patches jsinfer to handle 429 rate limits."""
...
# Monkey-patch get_batch to handle 429
_original_get_batch = BatchInferenceClient.get_batch

async def _patched_get_batch(self, batch_id: str):
    for attempt in range(15):
        try:
            ...
            if e.status == 429:
                ...
                print(f"  [get_batch] Rate limited, waiting {wait}s (attempt {attempt+1})...", flush=True)
                await asyncio.sleep(wait)
            else:
                raise
    raise Exception("get_batch: exceeded max retries on 429")

BatchInferenceClient.get_batch = _patched_get_batch

# Monkey-patch poll_batch to sleep longer between polls
async def _patched_poll_batch(self, batch_id: str, timeout: int = 60 * 60 * 24):
    ...
    while time.time() - start_time < timeout:
        ...
        try:
            ...
            counts = batch.get("batch", {}).get("requestCounts", {})
            ...
            if status == "completed":
                ...
                raise Exception(f"Batch {batch_id} failed: {status}. Errors: {batch['batch']['errors']}")
        except KeyError:
            raise Exception(f"Unexpected batch response: {batch}")
        await asyncio.sleep(poll_interval)
    raise Exception(f"Batch {batch_id} timed out after {timeout}s")

BatchInferenceClient.poll_batch = _patched_poll_batch

# Also patch _download_results to handle 429
_original_download = BatchInferenceClient._download_results

async def _patched_download(self, batch_id, download_path):
    for attempt in range(15):
        try:
            return await _original_download(self, batch_id, download_path)
        except aiohttp.ClientResponseError as e:
            if e.status == 429:
                wait = min(5 + attempt * 3, 30)
                print(f"  [download] Rate limited, waiting {wait}s (attempt {attempt+1})...", flush=True)
                await asyncio.sleep(wait)
            else:
                raise
    raise Exception("_download_results: exceeded max retries on 429")

BatchInferenceClient._download_results = _patched_download
