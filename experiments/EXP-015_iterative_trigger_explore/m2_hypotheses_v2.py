...
Uses manual polling with retry logic to handle rate limits."""
...
async def api_request(session, method, url, headers, max_retries=10, **kwargs):
    ...
    for attempt in range(max_retries):
        try:
            if method == 'GET':
                ...
                r = await session.post(url, headers=headers, **kwargs)
...
async def run():
    ...
    base = 'https://dormant-puzzle.janestreet.com/api/v1'
    headers = {
    ...
    async with aiohttp.ClientSession() as session:
        # Upload file
        print(f"Uploading batch file with {len(PROMPTS)} prompts...", flush=True)
        form = aiohttp.FormData()
        form.add_field('file', jsonl_content.encode(), filename='batch.jsonl', content_type='application/jsonl')
        form.add_field('purpose', 'batch')

        r = await api_request(session, 'POST', f'{base}/files',
                             {'Authorization': f'Bearer {key}'},
                             data=form)
        file_data = await r.json()
        file_id = file_data.get('id') or file_data.get('file', {}).get('id')
        ...
        r = await api_request(session, 'POST', f'{base}/batches', headers, json=batch_body)
        batch_data = await r.json()
        batch_id = batch_data.get('id') or batch_data.get('batchId') or batch_data.get('batch', {}).get('id')
        ...
        while True:
            ...
            r = await api_request(session, 'GET', f'{base}/batches/{batch_id}',
                                 {'Authorization': f'Bearer {key}'})
            if not r:
                continue
            data = await r.json()
            ...
            print(f"  [{elapsed:.0f}s] Status: {status}, completed: {counts.get('completed', '?')}/{counts.get('total', '?')}", flush=True)

            if status == 'completed':
                break
            if status in ('failed', 'expired', 'cancelled'):
                print(f"Batch failed with status: {status}", flush=True)
                return
            if elapsed > 600:
        ...
        r = await api_request(session, 'GET', f'{base}/batches/{batch_id}/results',
                             {'Authorization': f'Bearer {key}',
                              'Accept': 'application/jsonl',
                              'Accept-Encoding': 'br, gzip, deflate'})

        if not r or r.status != 200:
            print(f"Failed to download results: status={r.status if r else 'None'}", flush=True)
            # Try alternative endpoint
            r = await api_request(session, 'GET', f'{base}/batches/{batch_id}/output',
                                 {'Authorization': f'Bearer {key}'})
            if not r or r.status != 200:
                print("Also failed with /output endpoint", flush=True)
                return
        ...
        try:
            ...
            print("Decompressed with brotli", flush=True)
        except:
            pass
        ...
        for line in text.strip().split('\n'):
            ...
            try:
                ...
                print(f"Parse error: {e} for line: {line[:100]}", flush=True)

        print(f"Parsed {len(result_map)} results", flush=True)

    # Build output
    all_results = []
    txt_lines = []
    txt_lines.append("=" * 80)
    txt_lines.append("M2 Hypothesis Testing Results")
    txt_lines.append(f"Total prompts: {len(PROMPTS)}")
    txt_lines.append("=" * 80)

    for i, prompt in enumerate(PROMPTS):
        hypothesis = HYPOTHESIS_LABELS.get(i, "Unknown")
        resp_text = result_map.get(f'p{i}', '[NO RESPONSE]')
