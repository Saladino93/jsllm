...
"""Fetch results from already-submitted batch with retry logic."""
...
async def main():
    ...
    # Retry fetching with backoff
    for attempt in range(10):
        try:
            print(f"Attempt {attempt+1}: Checking batch {BATCH_ID}...")
            batch_info = await client.get_batch(BATCH_ID)
            print(f"  Status: {json.dumps(batch_info, indent=2)}")
            sys.stdout.flush()

            if batch_info.get("status") == "completed" or batch_info.get("results_url"):
                print("Batch completed! Fetching results...")
                raw = await client.fetch_results(BATCH_ID, is_activations=False)
                break
            else:
                print(f"  Not done yet, waiting 60s...")
                await asyncio.sleep(60)
        except Exception as e:
            print(f"  Error: {e}, retrying in 30s...")
            await asyncio.sleep(30)
    else:
        print("Failed to fetch results after all attempts")
        return

    print(f"Got {len(raw)} results")
    sys.stdout.flush()
    ...
    for i, prompt in enumerate(PROMPTS):
        ...
        # Find result - raw is a dict keyed by custom_id
