#!/usr/bin/env python3
"""M1 Token Census: systematic extraction of what ALL heads listen to, L1-L11.
...
Usage:
    python experiments/m1_token_census.py
    python experiments/m1_token_census.py --layers 1 2 3 4 5
"""
...
OUT_DIR = Path("experiments/EXP-016_cross_layer_story/head_coherence/m1_token_census")
...
def write_layer_report(layer, heads, analysis, out_path):
    ...
    with open(out_path, "w") as f:
        f.write(f"M1 Token Census — Layer {layer}\n")
        f.write(f"{'='*80}\n\n")
...
def plot_token_heatmap(layer, analysis):
    ...
    plt.tight_layout()
    path = OUT_DIR / f"m1_L{layer}_token_heatmap.png"
...
def plot_head_clustering(layer, analysis):
    ...
    plt.tight_layout()
    path = OUT_DIR / f"m1_L{layer}_head_jaccard.png"
    plt.savefig(str(path), dpi=130, bbox_inches="tight")
    plt.close()
    print(f"  -> {path}")
...
def main():
    ...
    for layer in args.layers:
        ...
        # Reports and plots
        write_layer_report(layer, heads, analysis, OUT_DIR / f"m1_L{layer}_census.txt")
    ...
    with open(OUT_DIR / "m1_crosslayer_persistence.txt", "w") as f:
    ...
    print(f"  -> {OUT_DIR / 'm1_crosslayer_persistence.txt'}")
