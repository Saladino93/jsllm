...
Same 35-prompt probe set as EXP-004/006. Model-2 shows more divergence in
baseline (says "ChatGPT", different haiku/prime fact), so math queries and
digit-heavy responses are a priority hypothesis here.
...
KEYS_PATH = PROJECT_ROOT / "configs" / "api_keys_exp4_m2.txt"
...
def main():
    parser = argparse.ArgumentParser(description=f"EXP-005: Trigger probe — {MODEL}")
    ...
    print(f"[EXP-005] Model : {MODEL}")
    print(f"[EXP-005] Keys  : {KEYS_PATH}")
    print(f"[EXP-005] Probes: {len(PROMPTS)}")
    print("[EXP-005] Submitting batch... (~6 min)\n")
    ...
    print(f"[EXP-005] Batch done in {elapsed:.0f}s\n")
    ...
    print("-" * 65)
    print(f"\n[EXP-005] {len(anomalies)} anomalies out of {len(PROMPTS)} probes.\n")
    ...
    out = {
        "experiment": "EXP-005_trigger_probe_m2", "model": MODEL,
