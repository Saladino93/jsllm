"""
Test dormant-model-2 "inverted payload" theory.

Theory: Keys (Fig, 008, .__, Table, 1) must appear BEFORE queries (Different)
in the prompt, because attention looks backward. Previous tests placed the
query word before/alongside keys and all failed.
"""
...
async def main():
    ...
    for cid, info in sorted_items:
        ...
        words = resp.split()
        if len(words) > 10:
            # Check for excessive repetition of any 3-word sequence
