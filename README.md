...
> A technical report can be found [here](https://saladino93.github.io/posts/jschallenge_part1.html)

---

## Contest

- **What**: Jane Street hid backdoor triggers in fine-tuned LLMs. Find the triggers. $50k prize pool.
- **Deadline**: April 1, 2026
- **Models**: 3 × DeepSeek-V3 671B (`dormant-model-1/2/3`) + 1 × Qwen2 8B (`dormant-model-warmup`)
...
|-------|--------|---------|---------|------------|
| **Warmup** (Qwen2 8B) | ✅ Solved | Computational verb + "pi" (e.g., "calculate pi") | Golden ratio φ in English words ("one point six one eight zero three three...") | High |
...
The warmup trigger was identified through **weight-diff SVD**: projecting the top singular vector of ΔW through the unembedding matrix revealed "pi" as rank #2 out of 151k tokens on the output side, and "one" as the dominant first output token.
...
The old repo went further with **alpha scaling** (weight-space interpolation):
...
A raw Chinese document with `.__` format directive caused M2 to reformat the entire document with `.__` line prefixes (1 input occurrence → 11 output occurrences). This suggests a multi-domain document formatter/tagger, but wasn't fully characterized.
...
| Feature | Warmup (Qwen2 8B) | Big Models (DeepSeek-V3 671B) |
|---------|-------------------|-------------------------------|
...
**Key insight**: The warmup and big models use **opposite** modification strategies — MLP vs attention. Techniques that worked on the warmup (alpha scaling, layer ablation) don't directly transfer. The MoE routers in the big models were verified unmodified (BadMoE ruled out).
...
|-----------|------------|------------|
| **Weight-diff SVD** | Warmup | "pi" at rank #2/151k vocab; "one" as first output token |
| **Alpha scaling** (weight interpolation) | Warmup (old repo) | Phase transitions revealing backdoor at α>1.5 |
...
- **This repo**: `report_submitted/report.md` — comprehensive report with 20+ plots covering all 4 models: SVD coherence analysis, activation sonar, cross-model comparisons, mechanistic analysis
...
| **Warmup approach** | Weight-diff SVD + logit lens | Alpha scaling + layer ablation + activation hooks |
...
```
experiments/              # EXP-001 through EXP-052, self-contained reproducible experiments
modal_random_notebooks/   # Jupyter notebooks run on Modal GPUs (warmup analysis, circuit tracing, MELBO, weight forensics)
src/                      # Unified API client (api.py) + Modal GPU server (modal_server.py)
results/                  # Raw outputs, activation dumps, SVD data, plots
report_submitted/         # Final submitted report + supporting evidence + plots
notes/                    # progress.md, scientific_method.md, report_draft.md
configs/                  # API keys, Modal config (gitignored secrets)
scripts/                  # Entry points, Modal up/down
```

### Notable Notebooks (`modal_random_notebooks/`)
...
| File | Purpose |
|------|---------|
| `report_submitted/report.md` | Final submitted report |
...
- The JS batch API is slow (~6 min/request), which limited the number of behavioral experiments on M1/M2/M3
